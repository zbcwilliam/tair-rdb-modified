...
void pushnGenericCommand(redisClient *c, int where) {
    ...
    list *list;
    listNode *ln;
    int argc_limit = c->argc;
    int argc_offset = 2;
    int list_length = 0;
    int should_push_length = c->argc - 2;
    if (lobj != NULL) {
       list_length = listTypeLength(lobj);
    }
    if (c->max_count > 0) {
        if (c->max_count < list_length + should_push_length) {
            int dlt = list_length + should_push_length - c->max_count;
            int temp = dlt - list_length;
            int should_pop = temp > 0 ? list_length : dlt;

            argc_offset += (temp > 0 ? temp : 0);
            /* Remove list elements to perform the trim */
            if (lobj != NULL) {
                if (lobj->encoding == REDIS_ENCODING_ZIPLIST) {
                    if (where == REDIS_TAIL) {
                        lobj->ptr = ziplistDeleteRange(lobj->ptr,0,should_pop);
                    } else {
                        lobj->ptr = ziplistDeleteRange(lobj->ptr,list_length - should_pop,should_pop);
                    }
                } else if (lobj->encoding == REDIS_ENCODING_LINKEDLIST) {
                    list = lobj->ptr;
                    int j;
                    if (where == REDIS_TAIL) {
                        for (j = 0; j < should_pop; j++) {
                            ln = listFirst(list);
                            listDelNode(list,ln);
                        }
                    } else {
    ...
    int cant_push_into = list_length + (c->argc - argc_offset) - c->server->list_max_size;
    cant_push_into = cant_push_into > 0 ? cant_push_into : 0;
    argc_limit = argc_limit - cant_push_into;
