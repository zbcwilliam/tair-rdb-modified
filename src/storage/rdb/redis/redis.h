...
#define REDIS_SAME_OBJECT_ERR               -24
...
#define REDIS_CRON_LOOP_LIMIT 100
...
#define REDIS_DB_LESS_MEMORY    1024
...
/* filter list */
#define FILTER_TYPE_FIELD  1
#define FILTER_TYPE_KEY    2
#define FILTER_TYPE_VALUE  4

typedef struct filterNode {
    int8_t type;  /* key or field */
    int len;
...
typedef struct dumpThreadInfo dumpThreadInfo;
typedef struct dumpThreadInfo loadThreadInfo;
...
#define REDIS_DB_STATUS_WORKING     0
#define REDIS_DB_STATUS_FORBIDDEN   1
...
typedef struct redisClient {
    ...
    /* max count, some command need length limit */
    int max_count;
...
struct filterNode* next_filter_node(struct filterListIterator* iter);
...
robj *createStringObject(char *ptr, size_t len, uint32_t logiclock, uint16_t version);
...
long long emptyDb();
redisDb* selectDb(redisServer *c, int id);
...
#define create_load_thread_info(server,filename,filenamelength,area) \
    create_dump_thread_info(server,filename,filenamelength,area)
#define free_load_thread_info(info) free_dump_thread_info(info)
struct dumpThreadInfo* create_dump_thread_info(redisServer *server, char* filename, int filenamelength, int area);
...
extern void profiler_begin(char* begin_desc);
extern void profiler_end();
