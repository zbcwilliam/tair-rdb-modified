...
static int hashTypeSimpleDel(robj *o, robj *key, dictEntry *de)
{
    int deleted;
    robj* ts_robj = dictGetEntryKey(de);
    unsigned int ts = sdslogiclock(key->ptr);
    unsigned int timestamp = sdslogiclock(ts_robj->ptr);
    if (ts > timestamp) {
                ...
                unsigned int *vlen)
{
    if (o->encoding == REDIS_ENCODING_ZIPMAP) {
        ...
        found = zipmapXGet((unsigned char**)(&(o->ptr)),key->ptr,
                sdslen(key->ptr),v,vlen,sdslogiclock(key->ptr));
        decrRefCount(key);
        if (!found) return -1;
    } else {
        dictEntry *de = dictFind(o->ptr,key);
        if (de == NULL) return -1;
        if (hashTypeSimpleDel(o, key, de) == -1) return -1;
...
int hashTypeExists(robj *o, robj *key) {
    if (o->encoding == REDIS_ENCODING_ZIPMAP) {
        key = getDecodedObject(key);
        if (zipmapXExists((unsigned char**)(&(o->ptr)),key->ptr,sdslen(key->ptr))) {
            decrRefCount(key);
            return 1;
        }
        decrRefCount(key);
        ...
        dictEntry* de = NULL;
        if ((de = dictFind(o->ptr,key)) != NULL) {
            if (hashTypeSimpleDel(o, key, de) == -1) return 0;
            return 1;
        }
    }
    return 0;
...
int hashTypeSet(redisClient *c, robj *o, robj *key, robj *value) {
    ...
    if (o->encoding == REDIS_ENCODING_ZIPMAP) {
        ...
        o->ptr = zipmapXSet(o->ptr,
            key->ptr,sdslen(key->ptr),sdslogiclock(key->ptr),
            value->ptr,sdslen(value->ptr), &update);
        decrRefCount(key);
        decrRefCount(value);

        /* Check if the zipmap needs to be upgraded to a real hash table */
        if (zipmapXLen(o->ptr) > c->server->hash_max_zipmap_entries)
            convertToRealHash(o);
    } else {
...
int hashTypeDelete(robj *o, robj *key) {
    ...
    if (o->encoding == REDIS_ENCODING_ZIPMAP) {
        key = getDecodedObject(key);
        o->ptr = zipmapXDel(o->ptr,key->ptr,sdslen(key->ptr), &deleted);
        decrRefCount(key);
    } else {
        deleted = dictDelete((dict*)o->ptr,key) == DICT_OK;
        /* Always check if the dictionary needs a resize after a delete. */
...
unsigned long hashTypeLength(robj *o) {
    return (o->encoding == REDIS_ENCODING_ZIPMAP) ?
        zipmapXLen((unsigned char*)o->ptr) : dictSize((dict*)o->ptr);
}
...
hashTypeIterator *hashTypeInitIterator(robj *subject) {
    ...
    if (hi->encoding == REDIS_ENCODING_ZIPMAP) {
        hi->zi = zipmapXRewind(subject->ptr);
...
int hashTypeNext(hashTypeIterator *hi) {
    if (hi->encoding == REDIS_ENCODING_ZIPMAP) {
        if ((hi->zi = zipmapXNext(hi->zi, &hi->zk, &hi->zklen,
            &hi->zv, &hi->zvlen, &hi->timestamp)) == NULL) return REDIS_ERR;
    } else {
...
void convertToRealHash(robj *o) {
    ...
    while((p = zipmapXNext(p,&key,&klen,&val,&vlen,&timestamp)) != NULL) {
        ...
        keyobj = createStringObject((char*)key,klen,timestamp,0);
...
void filterCommand(redisClient *c) {
    ...
    o = lookupKeyReadWithVersion(c->db,c->argv[1],NULL);
    if (o == NULL) {
        return;
    }
    if (checkType(c,o,REDIS_HASH)) {
    ...
    /* use it to filter */
    hashTypeGet(o,c->argv[2],&value,&v,&vlen);
}
