 ...
 *
 * Version: $Id: table_manager.hpp 85 2011-01-10 07:26:18Z maoqi@taobao.com $
 *
 * Authors:
...
namespace tair {
   ...
   public:
      ...
      void init_migrate_done_set(boost::dynamic_bitset<> &migrate_done_set,
              const std::vector<uint64_t> &current_state_table);

      void set_table_for_localmode();

      /////////////////////////////
      void do_update_table(uint64_t *new_server_table, size_t size, uint32_t table_version,
              uint32_t copy_count, uint32_t bucket_count);
