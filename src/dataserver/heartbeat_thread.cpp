 ...
 *
 * Version: $Id: heartbeat_thread.cpp 213 2011-08-24 06:54:25Z zongdai@taobao.com $
 *
 * Authors:
...
namespace tair {
   ...
   void heartbeat_thread::set_thread_parameter(tbnet::IServerAdapter *adaptor,
           tbnet::DefaultPacketStreamer *streamer, tair_manager *tair_mgr)
   {
         ...
         case TAIR_RESP_HEARTBEAT_PACKET: {
            ...
            if (need_set && resp->server_version >= TAIR_CONFIG_MIN_VERSION
               ...
               if(server_list != NULL) {
                  ...
                  server_table_updater *stu = new server_table_updater(tair_mgr, server_list, resp->server_list_count,
                          server_version, resp->data_need_move, resp->migrated_info, resp->copy_count, resp->bucket_count);
                  stu->start();
                  //stu->wait();
               } else {
